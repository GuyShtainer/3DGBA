...
- **🥽 v1.3 stereoscopic 3D — per-eye dual-game (2026-06-10), pending hw test:** with the physical 3D slider UP, the top screen shows **both games at once in stereo** — top game to the LEFT eye, bottom game to the RIGHT eye (`C2D_CreateScreenTarget(GFX_TOP, GFX_RIGHT)` + `gfxSet3D(true)`, slider-gated via `osGet3DSliderState()`). Slider DOWN = flat (right eye mirrors left), so the slider itself is the on/off. 3D is enabled only inside a play session (picker/splash stay flat). The right eye carries the game only (no HUD). Easy follow-up if wanted: a **pop-out depth** mode (same game both eyes, slider-scaled disparity).
- **🗂️ v1.1 field-menu + bag touch (2026-06-09), pending hw test:** completes the touch-everything suite. **General field menus** (overworld START, **YES/NO**, script multichoice — all the shared `sMenu` cursor): tap a row -> write `sMenu.cursorPos` (+ the START mirror) + A; detected by a live `sMenu.windowId` window + sane choice count. **Bag** (field or battle ITEM): tap a visible item row -> write the live ListMenu row (+26) + A (scroll/pocket-switch deferred; bottom row = CLOSE BAG). Added `gbacore_write16`. Spec: [docs/kb/gen3-menu-bag-touch-spec.md](docs/kb/gen3-menu-bag-touch-spec.md). **Risks:** the field-menu detection is a heuristic — watch it doesn't fire in the plain overworld and block walking (would be tightened to START-callback-only); LeafGreen ROM addresses are FireRed-derived (unverified) so these features may be inert on LG.
- **🧭 v1.1 smart-touch features (2026-06-09), pending hw test:** **in-battle party menu** touch (tap a Pokémon to switch — detects the party screen via gMain.callback2 + gPartyMenu.menuType; single vs double slot layouts; write gPartyMenu.slotId + A); **double-battle target select** (tap a battler — detect HandleInputChooseTarget via gBattlerControllerFuncs, write gMultiUsePlayerCursor + A, position->battler via gBattlerPositions, skips absent); and **BFS tap-to-walk pathfinding** (routes around walls using the live collision grid gBackupMapLayout, 33x33 search window) with **continuous re-targeting while touched/slid** — drag to steer, and on a bike the held screen tile maps to a moving world tile so you keep riding. Stall->replan->interact(A) fallback handles NPCs/ledges. Spec: [docs/kb/gen3-touch-features-spec.md](docs/kb/gen3-touch-features-spec.md). Risks: row anchor (5) ±1 needs hw calibration; LeafGreen new addresses FireRed-derived (unverified).
