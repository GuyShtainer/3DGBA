...
**Key code anchors (all verified live):** `projects/3DGBA/source/main.c` — `render_game` (322-366), `preTex`/`preTgt` offscreen target (448-452), `build_depth_grid`/`tdepth[10][15]` (281-307), `metatile_layer` (260-278), `DepthSnap`/OAM rects (214-219), `pop_eye`/`warp_scenery_eye` (248-256, 310-317), per-eye render block (823-851), `worstMs` budget timer (907). Verdict basis: `projects/3DGBA/docs/kb/emerald-3d-depth.md` §0/§1. GPU caps: `/opt/devkitpro/libctru/include/c3d/{texenv.h,light.h,fog.h,lightlut.h,proctex.h}`, `3ds/gpu/enums.h` (TEV 369-373, `GPU_BUMP_AS_TANG` 423), `3ds/gpu/shbin.h:11-12` (no fragment shader). Note: this document was not written to disk per instructions — it is returned here for the parent agent to place (suggested home: `projects/3DGBA/docs/kb/hd2d-rendering.md`, listed in `docs/kb/00-overview.md`).