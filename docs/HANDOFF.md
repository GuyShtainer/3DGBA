...
- **State right now:** The full HD-2D stack **M1–M4 + depth-model-v2 + UI-panel-pop is committed** (`657b6e1`→`210908b`). On top, **M4 time-of-day lighting is implemented, reviewed clean, and building** (`.3dsx`+`.cia` clean) but **UNCOMMITTED** (`main.c` only; last commit `210908b`). M4 = a day/night directional color grade over the overworld: a key light whose colour+direction track the clock (warm low dawn/dusk, bright neutral noon, dim blue night) shades the terrain by its surface normal (analytic, from the `tdepth` elevation field) via a 15×10 gouraud **MULTIPLY** mesh (`light_for_hour`/`light_vert`/`light_pass`; pure citro2d, ~150 quads/eye, no shader). "Light" menu toggle (grid grew to 2×9), `Settings.light` appended (size-tolerant load now spans all 4 historical sizes), overworld-gated, slider-independent. Adversarial review (2 lenses × 2 skeptics, completed) found **zero real defects**. **The study's literal M4 (AI-baked normal atlases) is intentionally NOT built** — it needs an offline AI model run + per-game tile-matching RE that can't be validated here, and the study itself flags it "the one to cut"; this is the honest analytic realization of M4's intent, AI-atlas left as a documented deferred upgrade. Hardware feedback round 2 ("blur STILL too much, overrides so much text") drove a rework: blur is now ONE half-res bounce (gentle ~2×2 soften, was quarter-res mush), alpha ceiling 47% (was 67%), sharp band widened to rows 48–112, and text-awareness became a **game-agnostic BG0 text-layer tilemap scan** (`band_text_scan` in `main.c`: gen-3 draws ALL textboxes/banners/menus on BG0 — verified, the standard textbox window sits at tile rows 15–18 on bg 0; any non-filler entries under a band kill that band's blur per-band, RAM signals kept as precise backups). Awaiting **hardware test round 3**. Remaining researched-not-built: HD-2D M3 bloom, co-op overworld, wireless link.
