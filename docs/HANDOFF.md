...
- **State right now:** HD-2D **M1 (text-aware DoF) + M2 (grid warp) + M3 (bloom) are committed** (`657b6e1`→`57d8ddf`). On top, **depth-model-v2 + UI-panel-pop is implemented, reviewed, fix-passed, and building** (`.3dsx`+`.cia` clean) but **UNCOMMITTED** (`main.c` only; last commit `57d8ddf`). v2 = hardware round-4 feedback: a continuous **ground-plane ramp** (bottom of frame closer→pops more), **elevation-aware** scenery depth (map-grid elevation nibble: a hilltop pops above the grass), **characters always float above the floor at their feet** (incl. elevation via `floor_at` — fixes "floor pops over the character"), and **BG0 window panels pop in any context** (dialogs/START/bag/party/menus) as clean shifted copies so text is sharp AND deep. An adversarial review workflow (3 lenses × 2 skeptics) found real defects; all fixed: panel detector now **gen-3-profile-gated** + overworld near-full-screen guard (no phantom full-frame pop on arbitrary GBA games), UI pop **samples the sharp-bilinear prescale** (crisp text, not NEAREST shimmer), `draw_pop` **clips to the frame box** (no letterbox bleed), quad-fallback ramp made self-consistent. Hardware feedback round 2 ("blur STILL too much, overrides so much text") drove a rework: blur is now ONE half-res bounce (gentle ~2×2 soften, was quarter-res mush), alpha ceiling 47% (was 67%), sharp band widened to rows 48–112, and text-awareness became a **game-agnostic BG0 text-layer tilemap scan** (`band_text_scan` in `main.c`: gen-3 draws ALL textboxes/banners/menus on BG0 — verified, the standard textbox window sits at tile rows 15–18 on bg 0; any non-filler entries under a band kill that band's blur per-band, RAM signals kept as precise backups). Awaiting **hardware test round 3**. Remaining researched-not-built: HD-2D M3 bloom, co-op overworld, wireless link.
...
1. **Commit depth-v2** (after the user OKs) — one `feat` commit for the model + UI pop, one `fix` for the review findings — then **hardware-test round 4** (install the fresh `dual-gba.cia`, 3D slider up): (a) **finer object gradient** — a hill/stairs should now pop ABOVE the grass beside it (elevation nibble), not the same height; (b) **floor no longer pops over the character** — the player/NPCs always sit in front of the ground they stand on, even on raised terrain; (c) **ground gradient** — the bottom of the screen should pop a touch more than the top; (d) **menus/bag/party/dialog now have depth** — the textbox/menu panel pops strongly toward you (`UIPOP3D_PX` 5px) while the **text stays crisp** (sharp-bilinear source); (e) confirm no weird vertical strip on the screen sides next to a dialog (letterbox-bleed fix) and no full-screen flat pop. `worstMs` HUD ≤16 ms. Tuning knobs added: `POP3D_RAMP_PX` (2.4), `POP3D_CHAR_PX` (2.0), `ENV3D_ELEV` (0.8/level), `UIPOP3D_PX` (5.0).
2. **Known limitation to mention if asked:** battle text panels don't pop — gen-3 battle BG0 is 32×64, so `bg0_scan` bails at its size guard (and the guard protects the full-screen-pop case). Out of scope for round 4.
3. Earlier tuning knobs still apply: `DOF_ALPHA` (0x78), `DOF_SHARP_Y0/Y1` (48/112), `BLOOM_THRESH` (0xC8), `BLOOM_GAIN` (0x90).
