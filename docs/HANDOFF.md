...
- **State right now:** **Committed through HD-2D M1–M4, depth round 5 (elevation A+B+C), and the wireless M1 netlink foundation** (`657b6e1`→`29a8158`). On top, **depth round 6 is implemented + building (`.3dsx`+`.cia` clean) but UNCOMMITTED** (`main.c` only; last commit `29a8158`); an adversarial review of it is running (task to fold before commit). Round 6 = four hardware notes: (1) **lighting kept BRIGHT** when focused — `light_for_hour` ambient raised to 0.88–1.0 so the day/night grade is a subtle tint, not a dimming (fixes "whole game darkened, only the route-sign UI-pop stayed bright"); (2) **dropped the special centre/player rectangle** — the `nspr==0` fixed-rect fallback is gone, the effect is per-sprite only; (3) **3D-slider-off ⇒ plain 2D** — DoF/bloom/lighting now gated on `s3dOn` (slider>0.03) like the pops, so everything reverts when the slider is down; (4) **per-sprite "standee"** — each NPC/player/sprite is drawn in horizontal strips with feet at ground depth and the head ramping to +`POP3D_STANDUP` (3.0px) out of the screen. **Still open from note 4:** tree/rock *tiles* (not OAM sprites) don't yet stand up — that needs a different approach than the continuous vertex grid (shared vertices can't do per-tile standup) + the "fill behind the tree" occlusion; deferred, see Next steps.
...
### Session — 2026-06-11 (round 6) — depth feedback: bright/slider-gated/standee

- **Hardware notes:** (1) focused game looked darkened, only the route-sign UI-pop stayed bright; (2) the centre/player was treated specially (a rectangle); (3) want 3D-slider-off ⇒ plain 2D; (4) want every object (NPC, rock, tree…) to stand up — feet at ground, head popping out.
- **Did (uncommitted→committed this session, `main.c`):**
  - **Note 1 — bright lighting:** `light_for_hour` ambient raised to 0.88–1.0; day/night is now a subtle colour tint, not a dimming. Tradeoff (confirmed by review, accepted): the directional slope shading mostly clips to white now — i.e. the lighting is a tint, not shading. That's the right call for "keep it bright"; the "Light" toggle still turns it off.
  - **Note 2 — no centre rectangle:** removed the `nspr==0` fixed-player-rect fallback in `pop_eye`; effect is per-sprite only.
  - **Note 3 — slider gates everything:** `s3dOn = slider3d>0.03` now ANDs into `dofPass`/`bloomPass`/`litPass` (and the existing `pop3d`/`uipop`), so slider-down = plain 2D, all FX off.
  - **Note 4 — per-sprite standee:** `pop_eye` draws each sprite in `ceil(h/8)` horizontal strips; feet disparity = `RAMP_AT(fy)+floorD`, head ramps to `+POP3D_STANDUP` (3.0px) → leans out of the screen, anchored at the ground. Verified strips tile exactly (no gaps). `POP3D_CHAR_PX` retired.
  - **Review** (1 lens × 2 skeptics): 1 LOW finding (the lighting-clip tradeoff above, accepted); the standee-anchor and removed-fallback claims were refuted. No fixes.
- **Still open (note 4, scenery):** tree/rock **tiles** (not OAM sprites) can't stand up via the continuous vertex grid (shared vertices) — needs foreground-tile standee overdraws + "fill a ground tile behind the tree" occlusion. Deferred; offered to the user.
- **Verify on hardware:** focused game bright; slider-off fully normal; NPCs/player lean out feet-anchored; `worstMs` (standee adds draws).
